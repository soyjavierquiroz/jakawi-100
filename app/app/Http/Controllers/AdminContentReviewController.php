...
class AdminContentReviewController extends Controller
{
    ...
    public function benefit(Request $request, Benefit $benefit)
    {
        ...
        DB::transaction(function () use ($benefit, $request, $data) {
            $fields = ['review_status' => match ($data['action']) {
                'approve' => 'approved', 'changes' => 'changes_requested', default => 'rejected'
            }, 'reviewed_at' => now(), 'reviewed_by_user_id' => $request->user()->id, 'review_notes' => $data['notes'] ?? null];
