...
export default function ExperienceShow({ experience }: { experience: Experience }) {
    ...
    return <><Head title={experience.title} /><main className="min-h-screen bg-background pb-28 text-foreground">
        <section className="mx-auto max-w-5xl">
            ...
            <div className="px-4 pt-6 sm:px-8 sm:pt-10">
                ...
                {location ? <div className="mt-6 grid gap-3 text-sm font-semibold sm:grid-cols-3">
                ...
                </div> : <p className="mt-6 text-sm font-semibold text-muted-foreground">Esta experiencia no tiene próximas fechas disponibles.</p>}
                ...
                {!canReserve && sessions.length > 0 ? <p className="mt-10 rounded-2xl bg-surface-muted p-5 text-sm leading-6 text-muted-foreground">La reserva para esta experiencia aún no está disponible. Consulta directamente con el organizador.</p> : null}
            </div>
        </section>
        {canReserve ? <div className="fixed right-0 bottom-0 left-0 z-20 border-t border-border bg-surface p-4 pb-[max(1rem,env(safe-area-inset-bottom))]"><div className="mx-auto max-w-5xl"><button type="button" onClick={() => setReservationOpen(true)} disabled={sessions.length > 1 && !selectedSession} className="min-h-13 w-full rounded-2xl bg-brand px-5 text-sm font-extrabold text-brand-foreground disabled:opacity-50 sm:max-w-sm">RESERVAR</button></div></div> : null}
