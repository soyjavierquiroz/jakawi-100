        ...
        <>
            ...
            <main className="min-h-screen bg-background px-4 py-6 text-foreground sm:px-6">
                <section className="mx-auto flex w-full max-w-xl flex-col gap-5">
                    ...
                    {reservations.length > 0 ? <div className="rounded-md border border-border bg-surface p-5"><h2 className="text-xl font-semibold">Mis reservas</h2><div className="mt-3 divide-y divide-border">{reservations.map((reservation) => <div key={reservation.public_id} className="py-3 text-sm"><p className="font-semibold">{reservation.experience}</p><p className="text-muted-foreground">{formatDate(reservation.starts_at)} · {reservation.venue} · {reservation.partner}</p><p className="mt-1">{{pending: 'Pendiente', confirmed: 'Confirmada', rejected: 'Rechazada', cancelled: 'Cancelada'}[reservation.status as keyof Record<string, string>]}</p>{reservation.can_cancel ? <button className="mt-2 text-sm underline" onClick={() => router.post(`/reservas/${reservation.public_id}/cancelar`)}>Cancelar reserva</button> : null}</div>)}</div></div> : null}
                </section>
            </main>
        </>
    );
