...
export function JakawiImage({ src, srcset = [], sizes, alt, className = '', loading = 'lazy', priority = false, fallbackClassName = '' }: {
    ...
    if (!src || failed) return <div role="img" aria-label={alt} className={`flex h-full w-full items-end border border-border bg-surface-muted p-5 text-sm font-extrabold tracking-[0.14em] text-muted-foreground ${fallbackClassName}`}>JAKAWI</div>;
